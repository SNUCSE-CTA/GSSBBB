...
namespace GraphLib::GraphSimilarity {
    class DFSGED : public GraphEditDistanceSolver {
    public:
        ...
        void Initialize() {
            ...
            inverse_assignment =
                    std::vector<std::vector<int> >(G2->GetNumVertices() + 2,
                                                   std::vector<int>(G2->GetNumVertices(), -1));
    ...
    class BinaryBranching : public DFSGED {
        ...
        void DFS(int u, int v) {
            ...
            while (true) {
                ...
                auto [uprime, vprime] = GetNextPair();
    ...
    class BinaryBranchingHighestContribution : public BinaryBranching {
        std::pair<int, int> GetNextPair() {
            ...
            for (int i = 0; i < G1->GetNumVertices(); i++) {
                ...
                // if (dis(gen) < THROW_AWAY_PROB) continue;
    ...
    class BinaryBranchingHighestWeightGap : public BinaryBranching {
        std::pair<int, int> GetNextPair() {
            ...
            for (int i = 0; i < G1->GetNumVertices(); i++) {
                ...
                int assigned_j = assignment[depth + 1][i];
                ...
                for (int j = 0; j < G2->GetNumVertices(); j++) {
                    ...
                    // int ii = inverse_assignment[depth+1][j];
                ...
                std::pair<int, int> new_score = {minimum_weight, G1->GetDegree(i)};
                if (new_score > score) {
                    score = new_score;
                    uprime = i;
                    vprime = assigned_j;
